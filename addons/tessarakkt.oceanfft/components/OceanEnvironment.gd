...
@export var underwater_gradient:Gradient
...
func _process(_delta):
	...
	var oceanHeight := ocean.get_wave_height(camera.global_position, 2)
	
	if oceanHeight > camera.global_position.y:
	...
	if not player_is_surfaced:
		var intensity = lerp(1, 0, absf(camera.global_position.y - oceanHeight) / 200)
		intensity = clampf(intensity, 0, 1)
		var test = underwater_gradient.sample(intensity)
		environment.background_energy_multiplier = test.r
		sky_light.light_energy = test.r
		environment.volumetric_fog_density = lerp(0.0075, 0.00001, test.r);
		# print(intensity)
